...
const Translatebox = () => {
  ...
  return (
    <div className="text-[#6e4555] px-20 flex flex-col text-lg items-center">
      <div className={`${progress >= 3 ? "hidden" : ""} absolute ${progress === 0 ? "bottom-32 left-1/4" : progress === 1 ? "bottom-1/4 left-16" : progress === 2 ? "bottom-[15%] left-16" : ""} transition-all duration-500 ease-in-out`}>
        <div className="relative">
        <img src={Bird.src || ""} className="w-60"/>
        <div className={` ${progress >= 3 ? "hidden" : ""} absolute top-0 transform translate-x-full font-semibold bg-neutral-600/10 px-6 py-3 rounded-xl`}>{`${progress === 0 ? "please pick a language from the dropdown menu" : progress === 1 ? "Which phrase would you like to learn in the new language? ": progress === 2 ? "Press the microphone button to test your pronounciation." : ""}`}</div>
        </div>
      </div>
      <div>
        {
          progress >= 3 && (
            ...
            >
              ...
              {
                progress === 4 &&
              <Button className=" text-xl bg-[#F5E3E0] hover:bg-[#ffbc9f] active:bg-[#c76c95] rounded-xl mt-5" onClick={() => setProgress(0)}>
                Go Again
              </Button>
              }
            </motion.div>
        ...
        {progress < 3 && (
          <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: 50, transition: { duration: 0.5 } }}
              transition={{ duration: 1, ease: "easeInOut", delay: progress === 3 ? 1 : 0 }}
            >
            <p>Target:</p>
            <Select
              onValueChange={(lang) => {
        ...
        <AnimatePresence>
          {progress >= 1 && progress < 3 && (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: 50, transition: { duration: 0.5} }}
              transition={{ duration: 1, ease: "easeInOut", delay: progress === 3 ? 0.5 : 0  }}
            >
        ...
        <AnimatePresence>
          {progress >= 2 && progress < 3 &&(
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: 50, transition: { duration: 0.5 } }}
              transition={{ duration: 1, ease: "easeInOut", delay: progress === 3 ? 0.1 : 0 }}
            >
        ...
        <AnimatePresence>
          {progress === 2 && (
            <motion.div
              ...
              transition={{ duration: 1, ease: "easeInOut" }}
            >
              <VoiceProvider onFinish={() => setProgress(3)} titleString={translation}/>
            </motion.div>
          )}
        </AnimatePresence>
      </div>
